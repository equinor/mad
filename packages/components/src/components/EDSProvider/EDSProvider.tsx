...
import { Density, Token } from "../../styling/types";
...
export const EDSProvider = (props: PropsWithChildren<EDSProviderProps>) => {
    const colorScheme = props.colorScheme === 'unspecified' ? 'light' : props.colorScheme;
    const token = useMemo(() => {
        const proxy = createTokenProxy(colorScheme, props.density);
        return JSON.parse(JSON.stringify(proxy)) as Token;
    }, [colorScheme, props.density]);
