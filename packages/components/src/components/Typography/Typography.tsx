...
const resolveColor = (color: ColorVariants | string | undefined): string | undefined => {
  if (!color) return undefined;
  return typographyColors[color as ColorVariants] ?? color;
};

const resolveTypography = (variantName: TypographyVariants, group: TypographyGroups | undefined): TypographyType | undefined => {
  ...
  if (!group && quickVariants[variantName as keyof QuickTypographyVariants]) {
    return quickVariants[variantName as keyof QuickTypographyVariants] as TypographyType
  }
  return (typography[group ?? "" as TypographyGroups] as any)[variantName] as TypographyType
};
...
const resolveVariantName = (
  ...
  `${variant}${bold ? '_bold' : ''}${italic ? '_italic' : ''}` as TypographyVariants
...
export const Typography = ({
  ...
  return (
    <Text {...rest} style={[style, { color: resolveColor(color) }, rest.style]}>{children}</Text>
  )
}