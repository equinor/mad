...
export const DotProgress = ({
    ...
    const bumpAnimation = (val: Animated.Value) => Animated.timing(val, {
        toValue: 1,
        duration: 300,
    ...
    const swipeAnimation = Animated.loop(Animated.sequence([
        bumpAnimation(bumpValue1),
        bumpAnimation(bumpValue2),
        bumpAnimation(bumpValue3),
    ]));

    const opacity = (val: Animated.Value) => val.interpolate({
        inputRange: [0, 0.25, 0.5, 0.75, 1],
        outputRange: [0.16, 1, 1, 1, 0.16],
        extrapolateRight: "clamp",
    });
    const radius = (val: Animated.Value) => val.interpolate({
        inputRange: [0, 0.25, 0.5, 0.75, 1],
        outputRange: [
            height / 2 * 0.8,
            height / 2 * 1.0,
...
type CircularProgressStyleProps = Pick<DotProgressProps,
    "color"
>
