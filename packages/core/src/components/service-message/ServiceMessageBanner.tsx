...
import {EDSStyleSheet, PressableHighlight, Typography, useStyles, CellSwipeItem } from '@equinor/mad-components';
...
export const ServiceMessageBanner = () => {
    ...
    const {serviceMessage, isDismissed, setIsDismissed, isExpanded, setIsExpanded, expansionEnabled, setExpansionEnabled, isError} = useServiceMessageState();
    ...
    return <DisableableSwipeable disabled={!dismissAndURLIsVisible} renderRightActions={() => <CellSwipeItem title="Remove" iconName="close" color="danger" onPress={() => setIsDismissed(true)}/>}>
        <PressableHighlight style={styles.container} onPress={onPressServiceMessage} disabled={!expansionEnabled}>
        <View style={{flex: 1}}>
           <Typography color={styles.text.color} numberOfLines={expansionEnabled && !isExpanded ? 2 : 0} onTextLayout={(e) => {if (e.nativeEvent.lines.length > 2) {setExpansionEnabled(true)}}}>
                {message}
            </Typography>
                {dismissAndURLIsVisible && <>
                    {url && <Typography group="interactive" variant='link' style={styles.text} onPress={() => void Linking.openURL(url)}>{url}</Typography>}
                    <Typography variant="label" bold color={styles.text.color} style={{marginTop: 8, textDecorationLine: 'underline'}}>Swipe left to remove</Typography>
                </>}
        </View>
