...
- 979b5bd: New prop for the `WorkOrderCell.Navigation` called `isBookmarked`. Can be used to display
  the `bookmark` and `bookmark-outline` icons.
...
- f73c165: Fixed validation for when the `not started` icon should be visible in the `WorkOrderCell`
  component
...
- 93cd63d: Added new value to the `WorkOrderType`. Prevent excess padding when the `maintenanceType`
  is not being used.
